...
def pars(day:int, month:int, year:int = 2025):
    with open("under_over/page_source.html", "r", encoding="utf-8") as file:
        html = file.read()
    ...
    if body:
        ...
        if table:
            ...
            for element in elements:
                ...
                if team_name or probability or prediction or scores:
                    if(probability):
                        if int(probability)>=60 and prediction == "Under":
                            results.append({
                                "Team": team_name,
            ...
            if results:
                ...
                file_path = os.path.join(base_dir, "mix_file", "file1.txt")
                with open(file_path, "w", encoding="utf-8") as output_file:
                    ...
                    if (count_false+count_true) > 0:
                        output_file.write("winrate:" + str("{:.2f}".format((count_true * 100)/(count_false+count_true))))
                    output_file.write("\n\n\nGnidlo Sergiy")
                # with open(f"under_over/statistic_per_day/{year:04d}-{month:02d}-{day:02d}.txt", "w", encoding="utf-8") as output_file:
                #     for result in results:
                #         output_file.write(f"Team: {result['Team']}, Probability: {result['Probability']}, Prediction: {result['Prediction']}, Scores: {result["Scores"]}, IsSuccess: {result["IsSuccess"]}\n")
                #     output_file.write(f"count True : {count_true},   count False : {count_false}, count None : {count_none} \n")
                #     if (count_false+count_true) > 0:
    ...
    return([count_true,count_false,count_none])
